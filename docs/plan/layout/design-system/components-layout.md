...
- 레이아웃 컴포넌트는 화면 배치와 반복 구조를 소유한다
- 비즈니스 정책과 데이터 mutation은 소유하지 않는다
- 모바일과 데스크톱의 구조 차이는 허용하되 정보 구조는 유지한다
...
| 디자인 규칙 | sticky 허용, 얇은 glass surface 가능 |
...
| 디자인 규칙 | popover 또는 dropdown 형태 허용 |
...
| --- | --- |
| 역할 | 로그아웃 액션 전용 UI |
| 디자인 규칙 | destructive 톤은 주되 과격하지 않게 조절 |
